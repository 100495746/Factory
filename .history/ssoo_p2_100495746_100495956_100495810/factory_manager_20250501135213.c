...
void *belt_wrapper(void *arg){
	...
	int result;
	id = process_manager(args->belt.id, args->belt.size, args->belt.items);
	if (id!=0){
		printf(stderr, "[ERROR][factory_manager] Process with id %d has finished with errors", )
	}
	printf("[OK][factory_manager] process_manager with id %d has been created", id);
