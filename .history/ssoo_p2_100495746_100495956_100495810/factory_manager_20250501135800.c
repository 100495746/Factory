...
void *belt_wrapper(void *arg){
	...
	if (result!=0){
		printf(stderr, "[ERROR][factory_manager] Process with id %d has finished with errors",args->belt.id );
	}
	printf("[OK][factory_manager] process_manager with id %d has been created", args->belt.id);
...
int main (int argc, const char * argv[] ){
	...
	for (int i =0; i<count; i++){
		...
		printf("[OK][factory_manager] process_manager with id %d has finished", old_belts[i].id);
	}
	free(old_belts);
	sem_destroy(&semaphore);
	printf("[OK][factory_manager]")
