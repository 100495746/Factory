...
int process_manager (int id, int belt_size, int items_to_produce ){
	...
	struct thread_args prod_arguments = {id, items_to_produce, 0};

	queue_init(belt_size);



	pthread_create(&threads[0],NULL, *Produce, (void *)elemento);
	pthread_create(&threads[1],NULL, *Consume, (void *)elemento);
