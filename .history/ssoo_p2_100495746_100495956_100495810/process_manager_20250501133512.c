...
int process_manager (int id, int belt_size, int items_to_produce ){
	...
	if(??){
		printf("[ERROR][process_manager] Arguments not valid with id %d", id);
		return -1;
	}
	...
	if(prod_args == NULL){
		printf("[ERROR][process_manager] There was an error executing process manager with id %d", id);
		return -1;
	}
	...
	printf("[OK][process_manager] Belt with id %d has been created with a maximum of %d threads", id, item);
