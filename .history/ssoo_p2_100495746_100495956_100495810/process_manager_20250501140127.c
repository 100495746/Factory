...
int process_manager (int id, int belt_size, int items_to_produce ){
	...
	printf("[OK][process_manager] Process with id %d waiting to produce %d elements", items_to_produce);
	...
	if(cons_args == NULL){
		printf(stderr,"[ERROR][process_manager] There was an error executing process manager with id %d\n", id);
		return -1;
	}
