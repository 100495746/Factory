...
int main (int argc, const char * argv[] ){
	...
	// Try to read one more number: if there's anything left, the file is invalid
	// Check if any extra data is left in the file (invalid format)
	// if fscanf tried to read a trio but didn't succeed fully, it's a malformed file
	if (!feof(f)) {
		fprintf(stderr, "[ERROR][factory_manager] Invalid file.\n");
		fclose(f);
		free(old_belts);
	...
	int c;
	while ((c = fgetc(f)) != EOF) {
		if (c != ' ' && c != '\n' && c != '\t' && c != '\r') {  // if it's not space, newline, tab...
			fprintf(stderr, "[ERROR][factory_manager] Invalid file.\n");
			fclose(f);
			free(old_belts);
			return -1;
