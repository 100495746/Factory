...
// not good idea... they were not using the same queue

// To create a queue
int queue_init(struct queue *q, int size_param) {
	q->elements = malloc(size_param * sizeof(struct element)); // allocate memory of "size" (quantity of) elements
	if (q->elements == NULL) {
...
int queue_put(struct queue *q, struct element* elem) {
	...
	while (queue_full(q)) {
		//printf("[DEBUG][queue] Waiting: queue is full...\n");
...
struct element* queue_get(struct queue *q) {
	...
	*consumed = q->elements[q->head]; // copy element
